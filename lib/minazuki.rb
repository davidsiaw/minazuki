...
module Minazuki
  ...
  class GlobalGenerator
    ...
    def initialize(file, resources, prefix)
      @resources = resources
      @file = file
      @prefix = prefix
    end
  ...
  class ResourceGenerator
    ...
    def initialize(file, name, resource, index, prefix)
      @name = name
      @resource = resource
      @index = index
      @file = file
  ...
  class Generator
    ...
    def generator_datadir
      File.join(Gem.loaded_specs['minazuki'].full_gem_path, 'data', 'minazuki', 'generators')
    end

    def generator_files
      Dir["#{generator_datadir}/**/*.erb"]
    end
    ...
    def generate_file!(template, file, name, resource, index)
      ...
      rg = ResourceGenerator.new(file, name, resource, index, generator_datadir)
    ...
    def generate_globals!
      general_files.each do |file|
        gen = GlobalGenerator.new(file, resources, generator_datadir)
        template = File.read(file)
